...
public class Alarm {
    ...
    public Alarm() 
    {
    waitlist = new LinkedList<Waiter>();//维护一个water队列，且在链表中储存，把线程和她的时间对应
	Machine.timer().setInterruptHandler(new Runnable() 
	{//调用回调函数
		public void run() { timerInterrupt(); }//500时钟中断一次
     ...
     * 然后再利用Iterator的hasNext来对阻塞队列中的进程进行循环判断，
     * 如果该进程的唤醒时间大于当前时间，则对下一个进程进行判断，否则就将
     * 该进程唤醒，然后从阻塞队列中将其移除，再对下一个进程进行判断，
     * 最后将当前进程由运行状态转换成就绪状态，运行下一个进程。
     *  
     * */
    public void timerInterrupt() 
    {            //应该在此处检查所有sleep到时间了吗，设置回调函数
    Waiter waiter;
   // System.out.println("我还是执行了的");
    for(int i=0;i<waitlist.size();i++)//将waterlist的对象全部遍历
    {
    	waiter=waitlist.remove();
    	if(waiter.wakeTime<= Machine.timer().getTime())//现在的时间
    		{
    		waiter.thread.ready();}//加入就绪队列
    	else
	...
	System.out.println("我是"+KThread.currentThread().getName()+",我应该在"+wakeTime+"醒来，我睡去的时间是"+Machine.timer().getTime());
	KThread.sleep();
    ...
    private static class LcTest implements Runnable //测试类
    {
    	...
    	public void run() 
    	{
    	    for (int i=0; i<3; i++) //遍历waitlist链表
    	    {
    	    ThreadedKernel.alarm.waitUntil(3000);//调用上面的waitUntil
    		System.out.println("我是"+KThread.currentThread().getName()+"现在我说"+i);
    		}
    	}
    ...
    public void selfTest() {
    	new KThread(new LcTest()).setName("lc1").fork();//创建一个叫 lc1的线程和tcb程序控制块，并且将他加入到ready队列
    	waitUntil(500000);//测试alarm类
