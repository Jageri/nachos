 ...
 *
 * <p>
 * In order to emulate the hardware, we need to keep track of（记录） all pending（悬而未决的）
 * interrupts the hardware devices would cause, and when they are supposed to
 * occur.
 *
 * <p>
 * This module also keeps track of simulated（仿真的） time. Time advances only when the
 * following occur:
 * <ul>
 ...
 *
 * <p>
 * As a result因此, unlike real hardware, interrupts (including time-slice（片，在此时间片） context
