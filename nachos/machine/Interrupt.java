...
/**
 * The <tt>Interrupt</tt> class emulates(仿真) low-level interrupt hardware. The
 * hardware provides a method (<tt>setStatus()</tt>) to enable or disable
 * interrupts.
 *
 * <p>
 * In order to emulate the hardware, we need to keep track of（记录） all
 * pending（悬而未决的） interrupts the hardware devices would cause, and when they are
 * supposed to occur.
 *
 * <p>
 * This module also keeps track of simulated（仿真的） time. Time advances only when
 * the following occur:
 * <ul>
 * <li>interrupts are enabled, when they were previously（曾经，预先） disabled
 * <li>a MIPS instruction（指令） is executed
 * </ul>
 *
 * <p>
 * As a result因此, unlike real hardware, interrupts (including
 * time-slice（片，在此时间片） context switches) cannot occur just anywhere in the code
 * where interrupts are enabled, but rather only at those places in the code
 * where simulated time advances (so that it becomes time for the hardware
 * simulation to invoke an interrupt handler).
 *
 * <p>
 ...
 * hardware. But even though Nachos can't always detect（察觉） when your program
